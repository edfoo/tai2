...
class TestConfigMigration:
    def test_migrates_legacy_flat_config(self) -> None:
        ...
        migrated = MarketService._migrate_launcher_config(legacy)

        # Operational fields preserved at top level
        assert migrated["mode"] == "launcher_only"
        assert migrated["notional_usd"] == 60.0
        assert migrated["schedule"] == "timer"

        # Strategy fields namespaced
        mr = migrated["strategies"]["mean_reversion"]
        assert mr["enabled"] is True
        assert mr["rsi_oversold"] == 25.0
        assert mr["rsi_overbought"] == 75.0
        assert mr["tp_pct"] == 2.0
        assert mr["sl_pct"] == 11.0
        assert mr["dynamic_tp"] is True
        assert mr["dynamic_tp_fraction"] == 0.7

        # Legacy keys removed
        assert "mean_reversion_enabled" not in migrated
        assert "rsi_oversold" not in migrated
        assert "tp_pct" not in migrated
        assert "sl_pct" not in migrated

    def test_new_format_config_not_modified(self) -> None:
        new_config = {
            "mode": "launcher_only",
            "notional_usd": 60.0,
            "strategies": {
        ...
        result = MarketService._migrate_launcher_config(new_config)
        assert result == new_config

    def test_empty_config_not_modified(self) -> None:
        result = MarketService._migrate_launcher_config({})
        assert result == {}

    def test_partial_legacy_config_migrates_only_present_keys(self) -> None:
        legacy = {
            "mode": "launcher_only",
            "mean_reversion_enabled": True,
            "rsi_oversold": 20.0,
        ...
        assert migrated["mode"] == "launcher_only"
        assert migrated["notional_usd"] == 50.0
        mr = migrated["strategies"]["mean_reversion"]
        assert mr["enabled"] is True
        assert mr["rsi_oversold"] == 20.0
        # Keys not present in legacy are not in migrated strategy config
        assert "rsi_overbought" not in mr
        assert "tp_pct" not in mr

    def test_merges_with_existing_strategies_namespace(self) -> None:
        config = {
            "mode": "launcher_only",
            "mean_reversion_enabled": True,
            "rsi_oversold": 25.0,
            "strategies": {
                "other_strategy": {"enabled": True},
            },
        }
        migrated = MarketService._migrate_launcher_config(config)

        # Other strategy preserved
        assert migrated["strategies"]["other_strategy"] == {"enabled": True}
        # Mean reversion added
        assert migrated["strategies"]["mean_reversion"]["enabled"] is True
        assert migrated["strategies"]["mean_reversion"]["rsi_oversold"] == 25.0

    def test_set_launcher_config_auto_migrates(self) -> None:
        ...
        service.set_launcher_config(legacy)

        # Internal config should be migrated
        assert "mean_reversion_enabled" not in service._launcher_config
        assert service._launcher_config["strategies"]["mean_reversion"]["enabled"] is True
        assert service._launcher_config["strategies"]["mean_reversion"]["rsi_oversold"] == 25.0

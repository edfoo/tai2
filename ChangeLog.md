...
## 2026-09-26
- Backtest trade-tape spread estimation: added `app/services/backtest/spread.py` with Corwin-Schultz (OHLCV high-low) and Roll (trade-tape covariance) bid-ask spread estimators, plus a new `slippage_mode="tape_spread"` that adds the estimated spread to the base bps. This replaces an arbitrary constant with a data-derived, time-varying spread estimate.
- Added `fetch_trade_history_records` to `okx_metrics.py` (paginated `/api/v5/market/history-trades`) and `HistoricalDataFetcher.fetch_trade_tape` with caching and provenance; the engine builds the spread series per symbol (OHLCV or tape) when `tape_spread` is selected.
- Threaded `spread_estimator`/`spread_window` through `BacktestConfig`, the engine, the runner builders, the REST API, the job manager, the BACKTEST UI, and the REST CLI (`--spread-estimator`, `--spread-window`); documented the mode in `BACKTEST.md` and the simulation assumptions.
- Noted the remaining limitation explicitly: spread can be estimated from the tape, but order-book depth/impact cannot (OKX exposes no historical depth), so true impact calibration stays data-limited.
- Backtest cross-margin support: `margin_mode` now accepts `cross` in addition to `isolated`. In cross mode the simulator reserves no per-position initial margin (account equity backs all positions), caps new entries by `equity × leverage − deployed notional`, and liquidates the whole account when equity falls to the aggregate maintenance requirement (`maintenance_margin_requirement` / `cross_margin_liquidation_triggered` / `_liquidate_cross_account`). Isolated remains the live default.
- Removed the engine's hard rejection of cross margin (now validates `isolated`/`cross`), threaded `margin_mode` into the `Simulator`, exposed it in the REST API, BACKTEST UI, and REST CLI (`--margin-mode`), and updated the simulation assumptions to describe both liquidation models.
- Added cross-margin tests (no per-position liquidation price, shared-equity multi-position entry, account-level liquidation closing all positions, healthy-account non-liquidation, tier-MMR maintenance requirement) and updated the old "cross margin rejected" test.
- Backtest P3 (expert decision workflow): added `app/services/backtest/research.py`, a pure, JSON-safe bundle builder that turns serialised baseline/grid/stress/holdout artifacts into an auditable analysis bundle with a screening verdict (`candidate` / `inconclusive` / `reject`).
- The bundle adds fold consistency (fraction of validation folds profitable after costs), profit concentration (largest per-symbol and per-trade share), stress survival (net-after-cost delta per adverse preset), and reuses `analyze_sweep` robustness to flag lone-spike optima.
- Added `scripts/backtest_research.py`, a REST-driven CLI that runs baseline → sweep → stress → bundle for one strategy and writes `research_bundle.json` + `research_bundle.md`.
- Documented the workflow in `BACKTEST.md` and marked P3 implemented in `docs/objectives/backtest_research_plan.md`; added `tests/test_backtest_research.py`.
